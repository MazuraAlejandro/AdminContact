...
$("#btn_cancel_edit").click(function(event) {
  $("#modal_edit_user").css({
    display: 'none',
    opacity: '0',
    transform:'scaleX(0.7)',
...
$("#btn_delet").click(function(event) {
  delet();
});
...
function create(){
  ...
  if ($.trim(identification).length > 0 && $.trim(name).length > 0 && $.trim(lastname).length > 0 && $.trim(address).length > 0 && $.trim(user).length > 0 && $.trim(pass).length > 0) {
      // Llenar variable array
      var parametros = {"identification":identification,"name":name,"lastname":lastname,"address":address,"user":user,"pass":pass,"option":"create"};
      // Metodo Ajax
      $.ajax({
        url: 'models/user.php',
        type: 'post',
      ...
      .done(function(data) {
        ...
        if (data == 1) {
          // Ocultar Modal
          $("#modal_add_user").css({display: 'none',opacity: '0',transform:'scaleX(0.7)',top:'250.516px'});
          ...
          var $toastContent = $('<span>Usuario guardado correctamente !</span>');
